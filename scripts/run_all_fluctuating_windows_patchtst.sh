#!/usr/bin/env bash
# 遍历所有 169 个有起伏的预测窗口，依次运行 main.py
# 每个窗口用不同的 test_end_date，预测结果保存在 ./predictions/ 下
#
# 用法：
#   bash scripts/run_all_fluctuating_windows.sh
#   bash scripts/run_all_fluctuating_windows.sh 2>&1 | tee logs/run_all_windows.log
...
model=PatchTST
seq_len=52
pred_len=13          # 与 find_fluctuating_window.py 保持一致
percent=100
gpt_layer=6
label_len=18
batch_size=16
